...
def DP(host_tree, parasite_tree, phi, locus_map, D, T, L, O, R):
    ...
    for ep in postorder(parasite_tree):
        ...
        c_choice = (EventType.SPECIES_LIST, ep)
        s_choice = (EventType.LOCATION_LIST, ep)
        o_event = (EventType.ORIGIN, ep)
...
def DTL_reconcile(host_tree, parasite_tree, phi, D, T, L):
    ...
    for ep in postorder(parasite_tree):
        ...
        for eh in postorder(host_tree):
            ...
            if vh_is_a_tip:
                if vp_is_a_tip and phi[ep] == eh:
                    e = (EventType.CONTEMPORANEOUS, None, None)
                    A[(ep, eh)] = (0, [e])
                else:
                    A[(ep, eh)] = (Infinity, [])
            else:
            ...
            # Duplication
            if not vp_is_a_tip:
                dup_event = (EventType.Duplication,
            ...
            if not vp_is_a_tip:
                ...
                ep2_switch_events = [(EventType.TRANSFER,
                    (NodeType.SPECIES_MAPPING, ep1, eh),
                    (NodeType.SPECIES_MAPPING, ep2, loc)) \
                ...
                ep1_switch_events = [(EventType.TRANSFER, 
                    (NodeType.SPECIES_MAPPING, ep2, eh),
                    (NodeType.SPECIES_MAPPING, ep1, loc)) \
            ...
            for event in C_graph[ep_eh_m]:
                ...
                if r is not None:
                    event_nodes.apppend(r)
                C_graph[event] = event_nodes
            C_star[ep] = find_min_events([C_star[ep], (C[ep_eh_m], eh)])
            # Compute O: (cost, [mapping_node])
            O_c = (C[(ep, eh)], [(NodeType.SPECIES_MAPPING, ep, eh)])
            if vh_is_a_tip:
                O[ep_eh_m] = O_c
            else:
                O[ep_eh_m] = find_min_events(O_c[ep_eh_m], O[ep_eh1_m], O[ep_eh2_m])
        # Compute best_switch
        ...
        species_choice = (EventType.SPECIES_LIST, ep)
        C_graph[species_choice] = C_star[ep]
...
def synteny_reconcile(host_tree, parasite_tree, locus_map, R):
    ...
    for ep in postorder(parasite_tree):
        ...
        for lp in allsynteny:
            if check_tip(vp, ep1, ep2):
                if locus_map[ep] == lp:
                    S[(NodeType.LOCATION_MAPPING, ep, lp)] = (0, [lp])
                else:
                ...
                S_graph[(NodeType.LOCATION_MAPPING, ep, lp)] = a_nodes
                S_star[ep] = find_min_events(S_star[ep], (cost, [lp]))
        # Create the appropriate choice node
        location_choice = (EventType.LOCATION_LIST, ep)
        S_graph[location_choice] = S_star[ep]
    return S, S_star

def find_MPR(G, MPR, rand=False):
    MPR = {}
    extant_nodes = [(EventType.ROOT)]
    while len(visited) != 0:
        node = visited.pop()
        next_nodes = G[node]
        if node.graph_type is GraphType.CHOOSE:
            if rand:
                choice = random.choice(next_nodes)
            else:
                choice = next_nodes[0]
            MPR[node] = choice
            extant_nodes.append(choice)
        elif node.graph_type is GraphType.ALL:
            MPR[node] = next_nodes
            extant_nodes.extend(next_nodes)
        elif node.graph_type is None:
            pass
        else:
